...
class AIChordService {
    ...
    async generateChordProgression(request) {
...
Use only standard chord notation (C, Cm, F, G, Am, etc.).`;
    ...
    normalizeChordName(chord) {
        ...
        if (/^[A-G][#b]?minor$/i.test(c))
            c = c[0].toUpperCase() + (c[1] || "") + "m";
        if (/^[A-G][#b]?major$/i.test(c))
            c = c[0].toUpperCase() + (c[1] || "");
        return c;
    }
    convertChordsToPianoKeys(chords) {
